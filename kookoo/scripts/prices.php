...
$res= $dbh->Query("SELECT * FROM `wheat` WHERE `district`='".$_SESSION['district']."' ORDER BY `date` ");
...
res= $dbh->Query("SELECT * FROM `rice` WHERE `district`='".$_SESSION['district']."' ORDER BY `date` ");
$row= $dbh->FetchRow($res);
if(mysql_num_rows($res)>=1)
{ $temp=1; $r->addPlayText("The latest price of rice in your district is ".$row['modal_price']." rupees per quintal"); }

res= $dbh->Query("SELECT * FROM `sugar` WHERE `district`='".$_SESSION['district']."' ORDER BY `date` ");
$row= $dbh->FetchRow($res);
if(mysql_num_rows($res)>=1)
